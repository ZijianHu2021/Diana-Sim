#!/usr/bin/env python3
"""
Edge-Weighted GCN模型定义（7层深层网络 + 128维隐层 + 边权重版本）

📊 模型架构: GCNNet7D_EW(3 → 128 → 128 → 128 → 64 → 64 → 64 → 1) + Edge Weights
...
class GCNNet(nn.Module):
    """
    完整Edge-Weighted GCN网络（7层 + 128维隐层 + 边权重），用于节点级预测
    ...
    架构:
      Layer 1: 3 → 128 (初步特征提取)
      Layer 2: 128 → 128 (邻域交互学习)
      Layer 3: 128 → 128 (深层特征融合)
      Layer 4: 128 → 64 (特征压缩)
      Layer 5: 64 → 64 (深层特征融合)
      Layer 6: 64 → 64 (深层特征融合)
      Layer 7: 64 → 1 (最终预测)
    ...
    def forward(self, x, edge_index, edge_attr=None):
        ...
        # 层5: 深层特征融合 (64 → 64维) + 边权重
        x = self.conv5(x, edge_index, edge_attr)
        x = self.relu(x)
        x = self.dropout(x)

        # 层6: 深层特征融合 (64 → 64维) + 边权重

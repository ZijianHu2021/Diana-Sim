#!/usr/bin/env python3
"""
训练Edge-Weighted GCN模型（7层深层 + 128维隐层 + 边权重版本）
包含特征归一化、详细日志、评价指标、预测结果保存和loss曲线可视化

🔑 关键改进（相比gcn5d）:
  ✅ 在训练/验证/测试时传入edge_attr
  ✅ 模型利用Jacobian边权重进行消息传递
  ✅ 期望获得更好的物理特性建模能力
  ✅ 7层深层网络，相比5层更深，学习能力更强
"""
import argparse
import random
import json
...
def main(config_path: str | None = None):
    ...
    log_msg(f"{'='*70}")
    log_msg(f"Edge-Weighted GCN训练日志（7层 + 128维隐层 + 边权重）- {training_timestamp}")
    ...
    log_msg(f"  【7层深层 + 128维隐层 + 边权重(Jacobian)版本】")
    ...
    metrics_data = {
        ...
        "model_version": "7-layer-deep-128d-edge-weighted",
        "edge_weighted": True,
        "epochs": epochs,
    ...
    fig.suptitle(f"Edge-Weighted GCN Training Results (7-Layer + 128D + Edge Weights) ({data_tag})\n" + 

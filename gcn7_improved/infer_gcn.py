#!/usr/bin/env python3
"""
Edge-Weighted GCN Inference Example (7层 + BatchNorm + 128维 + 边权重版本)
Inference and evaluation on test set

🔑 关键改进（相比gcn7）:
  ✅ 在推理时传入edge_attr
  ✅ 利用Jacobian边权重进行更准确的预测
  ✅ BatchNormalization稳定梯度传播
  ✅ 改善推理精度和泛化能力
"""
import pickle
import json
...
def main(data_tag: str = None, training_timestamp: str = None, suffix: str = "", model_path: Path = None, scaler_path: Path = None, config: dict = None, config_path: Path = None, metrics_path: Path = None):
    ...
    # Configuration
    output_dir = resolve_output_dir(config) if config else Path("/home/hu/Diana-Sim/gcn7_improved")
    ...
    if metrics_path:
        ...
        # Format: /home/hu/Diana-Sim/gcn7/logs/<device_data_timestamp>/<training_timestamp>/metrics_*.json
    ...
    log_msg("="*70)
    log_msg("Edge-Weighted GCN Inference Log (7-Layer + BatchNorm + 128D + Edge Weights)")
    log_msg("="*70)
    log_msg("")
...
if __name__ == "__main__":
    ...
    output_dir = Path("/home/hu/Diana-Sim/gcn7_improved")  # 默认输出目录

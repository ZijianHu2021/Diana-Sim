#!/usr/bin/env python3
"""
标准GCN Inference Example (5层 + 128维 + 不使用边权重版本)
...
def main(data_tag: str = None, training_timestamp: str = None, suffix: str = "", model_path: Path = None, scaler_path: Path = None, config: dict = None, config_path: Path = None, metrics_path: Path = None):
    ...
    # Configuration
    output_dir = resolve_output_dir(config) if config else Path("/home/hu/Diana-Sim/gcn5_woe")
    ...
    if metrics_path:
        ...
        # Format: /home/hu/Diana-Sim/gcn5_woe/logs/<device_data_timestamp>/<training_timestamp>/metrics_*.json
        ...
        _, data_timestamp, data_device = resolve_data_root(config) if config else (Path("/home/hu/Diana-Sim/graph/gnn_data"), "unknown", "unknown")
    ...
    log_msg("="*70)
    log_msg("Standard GCN Inference Log (5-Layer + 128D)")
    log_msg("="*70)
    log_msg("")
    ...
    log_msg(f"  - 使用标准GCN（对称归一化）")
    log_msg("")
    
    # Step 4: Perform inference
    log_msg("="*70)
    log_msg("[Step 4] Inference on Test Set (Standard GCN)")
    log_msg("="*70)
    ...
    # fig.suptitle(f"Standard GCN Test Set: Predictions vs True Labels [5-Layer + 128D]\n" + 
    ...
    fig2.suptitle(
        "Standard GCN Test Set: Input → Prediction → Label (Averaged per Node)\n"
        + "Node Mapping: "
    ...
    initial_guess_data = {
        ...
        "description": "Averaged predicted final voltages across all test samples (Standard GCN), to be used as initial guess for Newton iteration",
    ...
    log_msg(f"  - 模型: Standard GCN（使用对称归一化）")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Standard GCN Inference - Evaluate model on test set")
    ...
    output_dir = Path("/home/hu/Diana-Sim/gcn5_woe")  # 默认输出目录

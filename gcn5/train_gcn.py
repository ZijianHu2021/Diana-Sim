#!/usr/bin/env python3
"""
训练Edge-Weighted GCN模型（5层深层 + 128维隐层 + 边权重版本）
...
def main(config_path: str | None = None):
    ...
    log_msg(f"{'='*70}")
    log_msg(f"Edge-Weighted GCN训练日志（5层 + 128维隐层 + 边权重）- {training_timestamp}")
    ...
    log_msg(f"  【5层深层 + 128维隐层 + 边权重(Jacobian)版本】")
    ...
    metrics_data = {
        ...
        "model_version": "5-layer-deep-128d-edge-weighted",
        "edge_weighted": True,
        "epochs": epochs,
    ...
    fig.suptitle(f"Edge-Weighted GCN Training Results (5-Layer + 128D + Edge Weights) ({data_tag})\n" + 
